...
func Test_ExecWithRetries_Ok(t *testing.T) {
	...
	err := ExecLogOutput(l, "ls", time.Second * 0, "/home")
	assert.Nil(t, err)
	retryCallback := func(l *log.Logger, commandTimeout time.Duration) {
		l.Print("remounting")
	}
	err = ExecWithRetries(3, retryCallback, lRetry, "ls", time.Second * 0, "/home")
	assert.Nil(t, err)
...
func Test_ExecWithRetries_Fail(t *testing.T) {
	...
	err := ExecLogOutput(l, "ls", time.Second * 0, "/non_existing_path")
	assert.Error(t, err, "exit status 1")

	retryCallback := func(l *log.Logger, commandTimeout time.Duration) {
		l.Print("remounting")
	}
	err = ExecWithRetries(retries, retryCallback, lRetry, "ls", time.Second * 0, "/non_existing_path")
	assert.Error(t, err, "exit status 1")
